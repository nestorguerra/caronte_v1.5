...
class Dashboard {
    ...
    switchTab(tabId) {
        ...
        // Update Nav Button (finding by onclick attr logic is a bit brittle, simplified here)
        // Ideally we'd use data attributes, but for this demo this is fine.
        const buttons = document.querySelectorAll('.nav-btn');
        if (tabId === 'profile') buttons[0].classList.add('active');
        if (tabId === 'interview') buttons[1].classList.add('active');
        if (tabId === 'book') buttons[2].classList.add('active');
    }
            ...
            const html = `
                <div class="question-card">
                    ...
                    <textarea class="answer-area" id="q-${index}" 
                        oninput="dashboard.autoSaveAnswer(${index})" 
...
let dashboard;
function switchTab(tab) { dashboard.switchTab(tab); }
function handlePhotoUpload(input) { dashboard.handlePhotoUpload(input); }
function saveProfile(e) { dashboard.saveProfile(e); }
function downloadPDF(e) { dashboard.generatePDF(e); }
function downloadEPUB(e) { dashboard.generateEPUB(e); }
function logout() {
